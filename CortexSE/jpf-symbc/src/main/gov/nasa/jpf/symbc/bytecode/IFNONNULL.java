...
public class IFNONNULL extends gov.nasa.jpf.jvm.bytecode.IFNONNULL {
	...
	public Instruction execute (SystemState ss, KernelState ks, ThreadInfo ti) {
		...
		if(sym_v == null) { // the condition is concrete
			...
			//Nuno: original code
			/*System.out.println("gov.nasa.jpf.symbc.bytecode.IFNONNULL: condition is on a symbolic variable");
			ti.pop();
			
			return getTarget();*/
			
			//*******
			//Nuno: changed this to consider both branches when reference is null
			...
			if (conditionValue) {
				pc._addDet(Comparator.NE, sym_v, -1);
				if(!pc.simplify())  {// not satisfiable
					ss.setIgnored(true);
					System.out.println("[IFNONNULL] not satisfiable target");
				}else{
					//pc.solve();
					((PCChoiceGenerator) cg).setCurrentPC(pc);
					//System.out.println(((PCChoiceGenerator) cg).getCurrentPC());
					System.out.println("[IFNONNULL] get target");
				}
				...
				if (dp[0].equalsIgnoreCase("omega")) {// hack
					if((Integer)cg.getNextChoice()==0)
						pc._addDet(Comparator.GT, sym_v, -1);
					else {// ==2
						assert((Integer)cg.getNextChoice()==2);
						pc._addDet(Comparator.LT, sym_v, -1);
					}
				}
				else
					pc._addDet(Comparator.EQ, sym_v, -1);
				if(!pc.simplify())  {// not satisfiable
					ss.setIgnored(true);
					System.out.println("[IFNONNULL] not satisfiable next");
				}else{
					//pc.solve();
					((PCChoiceGenerator) cg).setCurrentPC(pc);
					//System.out.println(((PCChoiceGenerator) cg).getCurrentPC());
					System.out.println("[IFNONNULL] get next");
				}
				return getNext(ti);
			}
		}
