			...
			if (currentError != null){
				...
				if (!isEndState() && !isIgnoredState()) {
					...
					if (isNewState && depth >= depthLimit) {
						...
						if (newHState != null) { 
							//System.out.println("HS-NEW STATE "+vm.getCurrentThread().getName()+"-"+newHState.getStateId());
					...
					//System.out.println("HS-END STATE "+vm.getCurrentThread().getName()+"-"+getStateId());
				}
			}

			//if(isIgnoredState() && !this.getVM().getCurrentThread().isTerminated()){ //Nuno: added this condition; originally backtrackToParent was always executed
				//System.out.println("\t-- "+this.getStateId()+" backtrack to parent");
				backtrackToParent();
			//}
		}
